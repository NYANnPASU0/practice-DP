...
void Len(std::vector<std::vector<std::pair<int, int>>>& G, int start, int finish)
{
	std::set<std::pair<int, int>> st;
	std::pair <int, int> tmp1, tmp2;
	tmp1.first = 0;
	tmp1.second = start;
	std::vector<int>Mark(G.size()), Len_(G.size(), 1e9);
	Mark[start] = 1;
	st.insert(tmp1);
	while (st.size())
	{
...
int main()
{
	...
	while (true)
	{
		...
		else
		{
			...
			if (chisl == 1)
			{
				std::cout << "UFDTYHfFIJUSUDJSUEHF " << std::endl;
			}
			else
			{
				Dejkstra_stud();
